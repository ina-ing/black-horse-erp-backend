...
public record ApiResponse<T>(
    ...
    public static <T> ApiResponse<T> error(ErrorCode error) {
        return new ApiResponse<>(Instant.now(), error.getStatus(), error.getCode(), error.getMessage(), null);
    }

    public static <T> ApiResponse<T> error(ErrorCode error, T data) {
        return new ApiResponse<>(Instant.now(), error.getStatus(), error.getCode(), error.getMessage(), data);
    }

    public static <T> ApiResponse<T> error(ErrorCode error, String message) {
        return new ApiResponse<>(Instant.now(), error.getStatus(), error.getCode(), message, null);
    }
